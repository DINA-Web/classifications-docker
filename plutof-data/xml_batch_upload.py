...
headers = {"Content-type": "application/x-www-form-urlencoded",
           "Accept": "text/plain"}
params = urllib.urlencode({'client_id': 'e78ee90a3fabbd248e5f', #'4908fdadf35697117048',
                           'client_secret': '712a8c210c9830df541b9df28fa703b4503b0508', #'299191c8d310a3d818ac16f8f103711a3e968e14',
                           'grant_type': 'password',
                           'username': 'admin',
                           'password': 'password12',
                           'scope': 'write'})
...
print "Auth response %s %s" % (response.status, response.reason)
data = response.read()
json_data = json.loads(data)
access_token = json_data["access_token"]
token_type = json_data["token_type"]
token = token_type + ' ' + access_token
print "Proceeding to post data"
...
for p in sorted(ids_dict):
    ...
    print response.status, response.reason
    if response.status != 201:
      print "Aborted while posting to tree node id" 
      exit()
    data = response.read()
    json_data = json.loads(data)
    new_id = json_data["id"]
    ...
    if common_name_dict[p] != "":
        ...
        response = conn.getresponse()
        print response.status, response.reason, time.time()
        if response.status != 201:
            exit()
